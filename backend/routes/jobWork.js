...
router.get('/entries', protect, async (req, res) => {
  try {
    const entries = await JobWorkEntry.find().populate('jobWorkType').sort({ date: -1 });
    res.json(entries);
  } catch (err) {
    res.status(500).json({ message: "Server Error" });
  }
...
router.post('/entries', protect, async (req, res) => {
  try {
    ...
    if (!workType) return res.status(404).json({ message: "Job type not found" });
...
router.patch('/entries/:id/status', protect, async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin') {
      return res.status(403).json({ message: "Admin is not allowed to change the status." });
    }
